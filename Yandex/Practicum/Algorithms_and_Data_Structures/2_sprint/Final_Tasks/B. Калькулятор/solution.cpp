/*
	...
	-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
	Пусть глубина стека - L (учитываются только операнды). Все методы выполняются за No Amortize O(1).
	Итог: O(L) + O(1) + O(1) + O(1) = O(L);

	-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
	Пусть количество одновременно лежащих операндов в стеке - S, потому что пробелы и знаки операций не храню.
	Итог: O(S);
*/
