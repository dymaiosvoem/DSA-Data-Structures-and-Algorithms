/*
	...
	Методы:
		1) void push(const Participant& participant)
			-добавляю нового участника и делаю просеивание вверх;
			-при просеивании делаю обмены, если ребенок лучше родителя -> меняю их местами, продолжаю до тех пор, пока такие пары
			есть.
	...
	Построение кучи (N раз push): O(N * log N).
