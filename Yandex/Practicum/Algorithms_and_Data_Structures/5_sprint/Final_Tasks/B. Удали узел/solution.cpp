/*
    ...
    -- ВРЕМЕННАЯ СЛОЖНОСТЬ --
    Пусть H - высота дерева. Поиск удаляемой вершины идет вниз по одному ребру за шаг, поэтому не более O(H).

    Если у найденного узла 2 ребенка, то в ExtractSuccessor спускаюсь по правому поддереву до минимального элемента,
    что также занимает O(H).

    Итог: O(H) + O(H) = O(H).

    -- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
    Дополнительные контейнеры для хранения вершин не использую: O(1), кроме стека вызовов.

    Из-за рекурсии глубина стека в худшем случае = высоте дерева: O(H).
