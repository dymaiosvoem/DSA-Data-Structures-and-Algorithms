/*
	...
	В конструкторе размер хеш-таблицы задается как M = 2^17 корзин. это фиксированная константа, поэтому M не учитываю в O-нотации.
	...
	Для каждой операции put / get / delete_key:
		...
		-в равномерном распределении ключей, средняя длина цепочки L = O(1), поэтому операции будут работать за O(1).

	В худшем случае, когда все ключи попали в одну корзину, тогда одна операция будет выполняться за O(Q).
	...
	В хеш-таблице храню:
		-массив из M = 2^17 указателей на головы цепочек, это фиксированная константа;
		-число разных ключей не превосходит Q.
